...
// ── AI DESIGNER APIs (Cloudflare Worker) ──────────────────────────
...
export async function aiGenerateImage(prompt: string): Promise<string | null> {
  try {
    // Optimized prompt for high-quality t-shirt design (flat vector, bold lines, streetwear)
    // We append specific style tags to ensure professional results and reduce retries (cost-efficient)
    const enhancedPrompt = `${prompt}, bold flat vector t-shirt design, streetwear style, minimalist icon, high contrast, isolated on solid white background, 4k, professional graphic design, masterpiece`;
...
export async function aiReviewDesign(imageBlob: Blob): Promise<any> {
  try {
...
export async function aiSafetyCheck(imageBlob: Blob): Promise<any> {
  try {
    const form = new FormData();
    form.append("image", imageBlob);
